...
\underline{Consumption/Choice Set X}: the set of all alternative, or complete consumption plans, that the consumer can conceive -whether achievable or note
\begin{itemize}
...
\end{itemize}
\underline{Complements of Sets}: for a given subset $A \subseteq X$, we define the complement of $A$ in $X$ to be the set $A^{c} = \left\{ x \in X: x \notin A \right\}$
\begin{itemize}
...
\end{itemize}
\underline{Metric Distance}: in huge space $\mathbb{R}_{+}^{n}$, open balls (neighbourhoods) are determined using a metric distance function
\begin{itemize}
  ...
  \end{itemize}
\end{itemize}
\underline{Open Balls (neighborhoods)}: an open ball $B_{\varepsilon}(x) = \left\{ y \in \mathbb{R}_{+}^{n}: d(x,y) < \varepsilon \right\}$ is the set of points that are closer in Euclidean distance to $x \in \mathbb{R}_{+}^{n}$ than some real number $\varepsilon > 0$
\begin{itemize}
  ...
  \end{itemize}
\end{itemize}
\underline{Open Set}: a set $A \subseteq \mathbb{R}_{+}^{n}$ is open if and only if for each point $x \in A$ there is a radius $\varepsilon_{x} > 0$ such that the open ball $B_{\varepsilon}(x) \subseteq A$
\begin{itemize}
  ...
  \end{itemize}
\end{itemize}
\underline{Closed Set}: a set $A \subset \mathbb{R}_{+}^{n}$ is closed if and only if the complement $A^{c} = \left\{ x \in \mathbb{R}_{+}^{n}: \ x \notin A \right\}$ of A is an open set
\begin{itemize}
  ...
  \end{itemize}
\end{itemize}
\underline{DeMoran's Laws}: (Law 1) the complement of the union of any family of sets is equal to the intersection of the complements of the family, (Law 2) the complement of the intersection of any family of sets is equal to the union of the complements of the family
\begin{itemize}
  ...
  \end{itemize}
\end{itemize}
\underline{Bounded Set}: a set $A \subseteq \mathbb{R}_{+}^{n}$ is bounded if and only if we can put an open ball in $\mathbb{R}^{n}$ around the set $A$
\begin{itemize}
...
\end{itemize}
\underline{Convex Set}: a set $A \subseteq \mathbb{R}_{+}^{n}$ is a convex set if and only if for all $x, y \in A$, and each $\alpha \in [0,1]$, the point $\alpha x + (1- \alpha)y$ is also in A
\begin{itemize}
  ...
  \end{itemize}
\end{itemize}
\underline{Convergence}: for all $\varepsilon > 0$ there exists a $\mathbb{N}$ such that for all values of $n$ greater than $\mathbb{N}$ the $n^{th}$ \textit{elements value in the sequence minus the limit's value} will be less than $\varepsilon$
\begin{itemize}
...
\end{itemize}
\underline{Continuous Function}: a function $f: \mathbb{R}_{+}^{n} \rightarrow \mathbb{R}$ is continuous if and only if the inverse image $f^{-1} (B) = \left\{ x \in \mathbb{R}_{+}^{n}: f(x) \in B \right\}$ of each open ball $B$ in the range $\mathbb{R}$ is also open in the doman $\mathbb{R}_{+}^{n}$
\begin{itemize}
...
\underline{Preference Relation}: preferences are defined for each consumer over bundles of goods, with the notation $\succeq$ meaning "at least as good as"
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item  \underline{Example 4}: $\succeq = \left\{ (x,y) \in \mathbb{R}_{+}^{n} \times \mathbb{R}_{+}^{n}: \ \prod_{i=1}^{n} x_{i} \geq \prod_{i=1}^{n} y_{i} \right\}$ represents the case where all bundles are compared and on is at least as good as another whenever the product of quantities is higher. See below for a graphical representation where $x_{1}x_{2} = 2$
    \begin{itemize}
      \item  \includegraphics[width=8cm, height=6cm]{pic12}
    \end{itemize}
  \end{itemize}
\end{itemize}
\underline{Strict and Indifferent Preference Relations}
  \begin{itemize}
  ...
  \end{itemize}
\underline{Lexicographic Preferences}: describe preferences where an agent prefers any amount of one good (X) to any amount of another (Y), therefore if offered several bundles then the agent will choose the bundle that offers the most $X$ no matter how much $Y$ there is
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item  \includegraphics[width=8cm, height=6cm]{pic11}
    \end{itemize}
  ...
  \end{itemize}
\end{itemize}
\underline{Consumer Choice Axioms}: all bundles in the subset can be compared (completeness), choices are consistent (transitivity), the consumer can choose all bundles in the subset (continuity), and more is always better (strict monotonicity). These four axioms must hold for preferences to be considered 'rational'
\begin{itemize}
  \item  \underline{Completeness}: for all $x_{1}$ and $x_{2}$ in $X$, either $x_{1} \succeq x_{2}$ or $x_{2} \succeq x_{1}$
  \item  \underline{Transitivity}: for any three elements $x_{1}, x_{2}, x_{3}$ in $X$, if $x_{1} \succeq x_{2}$ and $x_{2} \succeq x_{3}$ then $x_{1} \succeq x_{3}$
  \begin{itemize}
    ...
    \end{itemize}
  \end{itemize}
  \item  \underline{Continuity}: for all $x \in \mathbb{R}_{+}^{n}$, the "at least as good as set" $\succeq (x)$ and the "no better than" set $\preceq (x)$ are closed in $\mathbb{R}_{+}^{n}$
  \begin{itemize}
    ...
    \begin{itemize}
      \item  \underline{Logic}: \begingroup\color{magenta} $SG(x)$ follows by definition of continuity and completeness when taking the complement of $W(x)$, $SW(x)$ follows by definition of continuity and completeness when taking the complement of $G(x)$  \endgroup
    \end{itemize}
  \end{itemize}
  \item  \underline{Strict Monotonicity}: for all $x_{0}, x_{1} \in \mathbb{R}_{+}^{n}$, if $x_{0} \geq x_{1}$ then $x_{0} \succeq x_{1}$ while if $x_{0} > x_{1}$ then $x_{0} \succ x_{1}$
  \begin{itemize}
  ...
  \end{itemize}
\end{itemize}
\underline{Intermediate Value Version of Continuity}: if the preferences are continuous and complete, then for any $x, y, z \in \mathbb{R}_{+}^{n}$, if $x \succ y$ and $y \succ x$, then there is some $\alpha \in (0,1)$ such that the convex combination $\alpha x + (1- \alpha)z$ is indifferent to y. This resembles the \textit{intermediate value theorem}
\begin{itemize}
  ...
  \begin{itemize}
    \item  \includegraphics[width=8cm, height=6cm]{pic9}
  \end{itemize}
\end{itemize}
\underline{Utility Representation}: a utility representation of preferences $\succeq$ is a real valued function $u: \mathbb{R}_{+}^{n} \rightarrow \mathbb{R}$ over the set of bundless such that for all bundles $x,y \in R_{+}$: \\ (a) if $x \succeq y$ then $u(x) \geq u(y)$ \\ (b) if $u(x) \geq u(y)$ then $x \succeq y$
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \begin{itemize}
        \item  \includegraphics[width=8cm, height=6cm]{pic10}
      \end{itemize}
...
\end{itemize}
\underline{Exercise 1}: which are transitive, complete, and continuous?
  \begin{itemize}
    \item  \underline{Example 1}: $x \succeq y$ if and only if $x_{1}x_{2} \geq y_{1}y_{2}$
    \item  \underline{Example 2}: $x \succeq y$ if and only if $(x_{1})^{2} + x_{2} \geq (y_{1})^{2} + y_{2}$
    \item  \underline{Example 3}: $x \succeq y$ if and only if (a) $x_{1} > y_{1}$ or (b) $x_{1} = y_{1}$ and $x_{2} \geq y_{2}$
    \item  \underline{Example 4}: $x \succeq y$ if and only if $(x_{1} - 3)^{2} + (x_{2} - 4)^{2} \geq (y_{1} - 3)^{2} + (y_{2}-4)^{2}$
  \end{itemize}
